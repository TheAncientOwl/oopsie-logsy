...
const action = {
  ok: EActionType.InvokeApplyRegexTagssOK,
...
> = {
  dispatch: tags => async (dispatch: TDispatch) => {
    ...
    try {
      const response = await invoke('apply_regex_tags', { tags });
      console.info(invokeApplyRegexTags.dispatch, `rust response: ${response}`);
      dispatch({ type: EActionType.InvokeApplyRegexTagssOK, payload: { tags } });
    } catch (error) {
