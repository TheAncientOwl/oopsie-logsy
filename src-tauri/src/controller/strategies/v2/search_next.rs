...
pub fn execute(
    ...
    if let Some(reader) = matches_reader.as_mut() {
        ...
        if out.search_index + 1 >= out.search_total {
            // Already at last entry
            out.has_prev = out.search_total > 1;
            out.has_next = false;
            return Ok(out);
        }
        ...
        reader
            ...
            .map_err(|e| format!("Failed to seek to next entry: {}", e))?;

        let mut buf = [0u8; 8];
        reader
            .read_exact(&mut buf)
            .map_err(|e| format!("Failed to read next record index: {}", e))?;
        ...
        out.has_prev = true;
        out.has_next = out.search_index - 1 < out.search_total;
    } else {
        log_assert!(
            &execute,
            false,
