...
pub fn execute(_data: &mut AppData, _desired_range: IndexRange) -> Result<LogsChunk, String> {
    let _log = ScopeLog::new(&execute);

    Err(String::from("V1 is deprecated"))

    // let active_tags = data.regex_tags.compute_active_tags();
    // let mut out = LogsChunk::new_with_field_capacity(active_tags.len(), desired_range.size());
    ...
    //             out.data[field_idx].push(field_reader.read_at(log_metadata.index));
    //             field_idx += 1;
    //         });
    //     }
    // }
